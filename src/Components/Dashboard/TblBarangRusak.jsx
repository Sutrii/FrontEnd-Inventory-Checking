...
function TblBarangRusak() {
  ...
  async function handleDelete(id) {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`http://localhost:8000/api/barang-rusak/${id}`);
  ...
  async function handleUpdate() {
    if (editData) {
      try {
        await axios.put(
          `http://localhost:8000/api/barang-rusak/${editData.id}`,
          editData
        );
  ...
  async function handleAddNew(newData) {
    try {
      await axios.post("http://localhost:8000/api/barang-rusak", newData);
