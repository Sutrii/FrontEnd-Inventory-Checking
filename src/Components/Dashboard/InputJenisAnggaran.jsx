...
const InputJenisAnggaran = ({ value, onChange }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [selectedJenisAnggaran, setSelectedJenisAnggaran] = useState(
    value || "Pilih Jenis Anggaran"
  );

  useEffect(() => {
    setSelectedJenisAnggaran(value || "Pilih Jenis Anggaran");
  }, [value]);
  ...
  const handleSelect = (jenis_anggaran) => {
    setSelectedJenisAnggaran(jenis_anggaran);
    setIsOpen(false);
    if (onChange) {
  ...
  return (
    <div className="w-full h-auto p-3 bg-white shadow-md rounded-2xl space-y-3">
      <div className="flex justify-between items-center">
        <h2 className="text-sm poppins-semibold">Jenis Anggaran</h2>
      </div>
...
export default InputJenisAnggaran;
